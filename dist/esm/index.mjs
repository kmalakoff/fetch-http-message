 ...
 */ export default function fetchHttpMessage(input, init) {
    ...
    if (headers !== undefined) {
        /* c8 ignore start */ if (hasHeaders && headers instanceof Headers) {
            for (const pair of headers.entries())lines.push(`${pair[0]}: ${pair[1]}`);
        } else {
            /* c8 ignore stop */ for(const key in headers)lines.push(`${key}: ${headers[key]}`);
        }
    }
    ...
    if (body !== undefined) {
        if (~[
            'GET',
            'HEAD'
        ].indexOf(method)) throw new Error(`Option body not valid with method ${method}`);
